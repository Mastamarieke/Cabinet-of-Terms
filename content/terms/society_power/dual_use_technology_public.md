---
term: Dual-Use Technology
cluster: Society & Power
analytical_layer: mechanism
status: publieksversie
version: V1
public_entry: dual_use_technology_public.md
analysis_version: pending
tags:
  - society-power
  - "Society & Power"
  - publieksversie
related_cause:
  - public/society_power/surveillance_capitalism_public
  - public/society_power/technofeudalism_public
related_mechanism:
  - public/society_power/digital_military_industrial_complex_public
related_consequence:
  - []
related_reaction:
  - public/design_philosophy/unblackboxing_public
---

## Navigation

**Layer:** Mechanism — Usable for both civilian and military purposes; boundary structurally porous

**Cause:** [[public/society_power/surveillance_capitalism_public|Surveillance Capitalism]] · [[public/society_power/technofeudalism_public|Technofeudalism]]
**Mechanism:** [[public/society_power/digital_military_industrial_complex_public|Digital-Military-Industrial Complex]]
**Consequence:** *—*
**Reaction:** [[public/design_philosophy/unblackboxing_public|Unblackboxing]]

**Public entry:** this document
**Deep analysis:** *not yet created*

---

> 🔍 **Quick Read** — 2 minutes · Public entry · Kabinet van Digitale Termen

---

# Dual-Use Technology

The GPS in your phone was developed by the military. The AI that writes your emails is trained on data that will also train drone targeting. This is not a coincidence.

**Literal meaning**
*Dual-Use* — having two uses. *Technology* — applied technical knowledge. Dual-Use Technology is technology developed for civilian purposes that has military applications, or vice versa.

**Origin**
Dual-Use Technology is a concept in arms control and export regulation. GPS, encryption, drones, and AI are documented dual-use technologies. The concept has expanded as the boundary between civilian and military applications has become structurally porous.

> Dual-Use Technology is technology whose civilian and military applications are structurally inseparable — making the distinction between commercial and weapons development increasingly theoretical.

**The Appeal**
Much foundational technology — the internet, GPS, advanced materials — originated in military research and has produced enormous civilian benefit.

**The Friction**
[[public/society_power/digital_military_industrial_complex_public|Digital-Military-Industrial Complex]] is the institutional expression. The boundary between civilian and military use is not a design choice — it is structurally absent.

**Why This Matters**
Once you see Dual-Use Technology as a structural condition, you see that the civilian technology stack and the military technology stack are not separate — and that the former is continuously being absorbed by the latter.

**See also:** [[public/society_power/digital_military_industrial_complex_public|Digital-Military-Industrial Complex]] · [[public/society_power/military_industrial_complex_public|Military-Industrial Complex]]

`#society-power` `#power` `#framing` `#digital-alertness`

---

📖 **Read more**
- [Dual-use technology](https://en.wikipedia.org/wiki/Dual-use_technology) — Wikipedia

---

*Generated with AI (Claude, Anthropic) using cartographic prompting — a research method developed within Project Digitale Alertheid, HAN CMD, 2026.*
