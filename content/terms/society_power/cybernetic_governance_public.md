---
term: Cybernetic Governance
cluster: Society & Power
analytical_layer: mechanism
status: publieksversie
version: V1
public_entry: cybernetic_governance_public.md
analysis_version: pending
tags:
  - society-power
  - "Society & Power"
  - publieksversie
related_cause:
  - public/society_power/surveillance_capitalism_public
  - public/society_power/technofeudalism_public
related_mechanism:
  - public/privacy_compliance/algorithmic_violence_public
related_consequence:
  - []
related_reaction:
  - public/design_philosophy/unblackboxing_public
---

## Navigation

**Layer:** Mechanism — Governance through algorithms and automated decisions

**Cause:** [[public/society_power/surveillance_capitalism_public|Surveillance Capitalism]] · [[public/society_power/technofeudalism_public|Technofeudalism]]
**Mechanism:** [[public/privacy_compliance/algorithmic_violence_public|Algorithmic Violence]]
**Consequence:** *—*
**Reaction:** [[public/design_philosophy/unblackboxing_public|Unblackboxing]]

**Public entry:** this document
**Deep analysis:** *not yet created*

---

> 🔍 **Quick Read** — 2 minutes · Public entry · Kabinet van Digitale Termen

---

# Cybernetic Governance

The decision was made by the system. No one is responsible. No one can be appealed to.

**Literal meaning**
*Cybernetic* — relating to regulatory systems, from Greek kubernetes (steersman). *Governance* — systems by which power is exercised. Cybernetic Governance describes the replacement of human decision-making with automated algorithmic systems.

**Origin**
The concept builds on Norbert Wiener's cybernetics (1948). Contemporary cybernetic governance describes welfare algorithms, border control systems, predictive policing, and automated content moderation — government functions outsourced to automated decision-making.

> Cybernetic Governance is the replacement of human judgment in governance with automated algorithmic systems — removing accountability while maintaining power.

**The Appeal**
Automated governance promises consistency, efficiency, and freedom from human bias. Processing millions of welfare claims by algorithm is faster and cheaper than human review.

**The Friction**
[[public/privacy_compliance/algorithmic_violence_public|Algorithmic Violence]] is the consequence: automated systems produce harm without individual accountability. The appeal of efficiency conceals the removal of contestability: you cannot argue with an algorithm.

**Why This Matters**
Once you see Cybernetic Governance as the replacement of accountability rather than improvement of efficiency, you see that the question is not whether the system is accurate — it is who bears the cost when it is wrong.

**See also:** [[public/privacy_compliance/algorithmic_violence_public|Algorithmic Violence]] · [[public/society_power/surveillance_capitalism_public|Surveillance Capitalism]] · [[public/design_philosophy/unblackboxing_public|Unblackboxing]]

`#society-power` `#power` `#framing` `#digital-alertness`

---

📖 **Read more**
- [Algorithmic governance](https://en.wikipedia.org/wiki/Algorithmic_governance) — Wikipedia

---

*Generated with AI (Claude, Anthropic) using cartographic prompting — a research method developed within Project Digitale Alertheid, HAN CMD, 2026.*
