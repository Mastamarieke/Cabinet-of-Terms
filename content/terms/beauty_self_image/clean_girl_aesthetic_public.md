---
term: Clean Girl Aesthetic
cluster: Beauty, Influencers & Self-Image
analytical_layer: mechanism
status: publieksversie
version: V1
public_entry: clean_girl_aesthetic_public.md
analysis_version: pending
tags:
  - beauty-influencers-self-image
  - "Beauty, Influencers & Self-Image"
  - publieksversie
related_cause:
  - public/platform_mechanisms/attention_economy_public
  - public/society_power/surveillance_capitalism_public
related_mechanism:
  - public/platform_mechanisms/recommender_systems_public
  - public/beauty_self_image/that_girl_public
related_consequence:
  - public/beauty_self_image/comparison_culture_public
related_reaction:
  - public/beauty_self_image/deinfluencing_public
---

## Navigation

**Layer:** Mechanism — Minimalist beauty as lifestyle and norm

**Cause:** [[terms/platform_mechanisms/attention_economy_public|Attention Economy]] · [[terms/society_power/surveillance_capitalism_public|Surveillance Capitalism]]
**Mechanism:** [[terms/platform_mechanisms/recommender_systems_public|Recommender Systems]] · [[terms/beauty_self_image/that_girl_public|That Girl]]
**Consequence:** [[terms/beauty_self_image/comparison_culture_public|Comparison Culture]]
**Reaction:** [[terms/beauty_self_image/deinfluencing_public|Deinfluencing]]

**Public entry:** this document
**Deep analysis:** *not yet created*

---

> 🔍 **Quick Read** — 2 minutes · Public entry · Kabinet van Digitale Termen

---

# Clean Girl Aesthetic

No make-up. Slicked bun. Gold hoops. Glowing skin. This is effortless. It took two hours.

**Literal meaning**
*Clean* — pure, unadorned, natural-looking. *Girl* — gendered. *Aesthetic* — a visual style. The Clean Girl Aesthetic is a beauty trend that presents minimal, "natural" appearance as the aspirational norm.

**Origin**
The Clean Girl Aesthetic emerged on TikTok around 2022, characterised by slicked-back hair, glowing skin, gold jewellery, and minimal visible make-up. It was documented as a mainstream beauty trend but also critiqued for disproportionately featuring specific racial and body types as its standard.

> The Clean Girl Aesthetic is the beauty norm that presents a specific, effortful appearance as natural and minimal — distributing a particular standard of beauty as universal aspiration.

**The Appeal**
Minimal, "natural" beauty is appealing as a counter to heavily produced aesthetics. The style is genuinely versatile and accessible.

**The Friction**
The "effortless" appearance requires significant product, time, and labour. [[terms/beauty_self_image/comparison_culture_public|Comparison Culture]] amplifies the standard: the feed produces the impression that this is how everyone looks naturally. The aesthetic has been critiqued for centring a specific body type as the baseline "clean."

**Why This Matters**
Once you see the Clean Girl Aesthetic as a constructed norm rather than a natural baseline, you see that the labour it hides is also the standard it sets — and that "effortless" is always effortful for someone.

**See also:** [[terms/beauty_self_image/comparison_culture_public|Comparison Culture]] · [[terms/beauty_self_image/that_girl_public|That Girl]] · [[terms/beauty_self_image/beauty_filter_public|Beauty Filter]]

`#beauty-influencers-self-image` `#power` `#framing` `#digital-alertness`

---

📖 **Read more**
- [Clean girl aesthetic](https://en.wikipedia.org/wiki/Clean_girl_aesthetic) — Wikipedia

---

*Generated with AI (Claude, Anthropic) using cartographic prompting — a research method developed within Project Digitale Alertheid, HAN CMD, 2026.*
