---
term: Brussels Effect
cluster: Privacy & Compliance
analytical_layer: mechanism
status: publieksversie
version: V1
public_entry: brussels_effect_public.md
analysis_version: pending
tags:
  - privacy-compliance
  - "Privacy & Compliance"
  - publieksversie
related_cause:
  - public/society_power/surveillance_capitalism_public
  - public/society_power/technofeudalism_public
related_mechanism:
  - public/privacy_compliance/vendor_lock_in_public
related_consequence:
  - []
related_reaction:
  - []
---

## Navigation

**Layer:** Mechanism — EU regulation as de facto global standard

**Cause:** [[terms/society_power/surveillance_capitalism_public|Surveillance Capitalism]] · [[terms/society_power/technofeudalism_public|Technofeudalism]]
**Mechanism:** [[terms/privacy_compliance/vendor_lock_in_public|Vendor Lock-in]]
**Consequence:** *—*
**Reaction:** *—*

**Public entry:** this document
**Deep analysis:** *not yet created*

---

> 🔍 **Quick Read** — 2 minutes · Public entry · Kabinet van Digitale Termen

---

# Brussels Effect

The EU passed a law. A company in California changed its global product.

**Literal meaning**
*Brussels* — seat of EU institutions. *Effect* — unilateral regulatory influence. The Brussels Effect describes how EU regulation becomes a de facto global standard because global companies find it easier to comply universally than to maintain separate products for different jurisdictions.

**Origin**
Anu Bradford coined the term in a 2012 article and developed it in *The Brussels Effect* (2020). She documented how GDPR, REACH chemical regulations, and other EU frameworks had effectively become global standards — not through international agreement but through market logic.

> The Brussels Effect is the mechanism by which EU regulation becomes a de facto global standard — because multinational companies apply the strictest standard universally rather than maintaining separate compliant products.

**The Appeal**
The Brussels Effect extends EU consumer protections to non-EU users without requiring global treaty negotiations. It is regulatory power without proportional political power.

**The Friction**
The Brussels Effect depends on companies valuing EU market access enough to comply. For companies that treat EU fines as a cost of doing business — as Meta has repeatedly demonstrated — the effect is limited. [[terms/privacy_compliance/privacy_washing_public|Privacy Washing]] is partly a response: surface compliance without behavioural change.

**Why This Matters**
Once you see the Brussels Effect as a mechanism of regulatory leverage rather than a democratic achievement, you see both its power — it has genuinely shaped global data practices — and its limits: it works through market logic, not democratic accountability.

**See also:** [[terms/privacy_compliance/privacy_washing_public|Privacy Washing]] · [[terms/privacy_compliance/vendor_lock_in_public|Vendor Lock-in]] · [[terms/society_power/surveillance_capitalism_public|Surveillance Capitalism]]

`#privacy-compliance` `#power` `#framing` `#digital-alertness`

---

📖 **Read more**
- Bradford, A. (2020): *The Brussels Effect*. Oxford University Press
- [Brussels effect](https://en.wikipedia.org/wiki/Brussels_effect) — Wikipedia

---

*Generated with AI (Claude, Anthropic) using cartographic prompting — a research method developed within Project Digitale Alertheid, HAN CMD, 2026.*
