---
term: Deadnaming
cluster: Culture Wars & Political Language
analytical_layer: consequence
status: publieksversie
version: V1
public_entry: deadnaming_public.md
analysis_version: pending
tags:
  - culture-wars-political-language
  - "Culture Wars & Political Language"
  - publieksversie
related_cause:
  - public/society_power/surveillance_capitalism_public
  - public/platform_mechanisms/recommender_systems_public
related_mechanism:
  - public/privacy_compliance/privacy_washing_public
related_consequence:
  - []
related_reaction:
  - public/culture_wars/deplatforming_public
---

## Navigation

**Layer:** Consequence — Old names remain findable; right to be forgotten affects identity

**Cause:** [[terms/society_power/surveillance_capitalism_public|Surveillance Capitalism]] · [[terms/platform_mechanisms/recommender_systems_public|Recommender Systems]]
**Mechanism:** [[terms/privacy_compliance/privacy_washing_public|Privacy Washing]]
**Consequence:** *—*
**Reaction:** [[terms/culture_wars/deplatforming_public|Deplatforming]]

**Public entry:** this document
**Deep analysis:** *not yet created*

---

> 🔍 **Quick Read** — 2 minutes · Public entry · Kabinet van Digitale Termen

---

# Deadnaming

The name you no longer use is still indexed. The search engine does not forget.

**Literal meaning**
*Dead name* — the birth name used before a transgender person's transition. *Deadnaming* — using that name after the person has adopted a new name.

**Origin**
Deadnaming entered LGBTQ+ and mainstream discourse in the 2010s. The term describes both interpersonal behaviour and a structural problem: digital permanence means old names persist in archives even after legal name changes.

> Deadnaming is the use of a transgender person's pre-transition name — a behaviour that is both interpersonal and infrastructural, as digital systems preserve old names indefinitely.

**The Appeal**
This term has no "appeal" in the standard sense. Its analytical value is in revealing how digital permanence intersects with identity.

**The Friction**
[[terms/privacy_compliance/privacy_washing_public|Privacy Washing]] is relevant: the right to be forgotten is legally established in GDPR but difficult to enforce against search indexes and archives. Deadnaming is partly a technical problem: systems that treat names as permanent identifiers are not designed with identity change in mind.

**Why This Matters**
Once you see deadnaming as a structural infrastructure problem, you see that the design of identity systems has direct consequences for lived safety.

**See also:** [[terms/privacy_compliance/privacy_washing_public|Privacy Washing]] · [[terms/culture_wars/dehumanization_public|Dehumanization]]

`#culture-wars-political-language` `#power` `#framing` `#digital-alertness`

---

📖 **Read more**
- [Deadnaming](https://en.wikipedia.org/wiki/Deadnaming) — Wikipedia

---

*Generated with AI (Claude, Anthropic) using cartographic prompting — a research method developed within Project Digitale Alertheid, HAN CMD, 2026.*
