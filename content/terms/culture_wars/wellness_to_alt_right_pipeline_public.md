---
term: Wellness-to-Alt-Right Pipeline
cluster: Culture Wars & Political Language
analytical_layer: mechanism
status: publieksversie
version: V1
public_entry: wellness_to_alt_right_pipeline_public.md
analysis_version: pending
tags:
  - culture-wars-political-language
  - "Culture Wars & Political Language"
  - publieksversie
related_cause:
  - public/platform_mechanisms/recommender_systems_public
  - public/platform_mechanisms/attention_economy_public
related_mechanism:
  - public/culture_wars/conspirituality_public
related_consequence:
  - []
related_reaction:
  - public/culture_wars/deplatforming_public
---

## Navigation

**Layer:** Mechanism — Trajectory from yoga/detox to far-right ideology via shared enemies

**Cause:** [[public/platform_mechanisms/recommender_systems_public|Recommender Systems]] · [[public/platform_mechanisms/attention_economy_public|Attention Economy]]
**Mechanism:** [[public/culture_wars/conspirituality_public|Conspirituality]]
**Consequence:** *—*
**Reaction:** [[public/culture_wars/deplatforming_public|Deplatforming]]

**Public entry:** this document
**Deep analysis:** *not yet created*

---

> 🔍 **Quick Read** — 2 minutes · Public entry · Kabinet van Digitale Termen

---

# Wellness-to-Alt-Right Pipeline

The entry point was gluten. The exit point was the Great Replacement. The algorithm built the bridge.

**Literal meaning**
*Wellness* — health and self-care practices. *Alt-Right* — far-right political ideology. *Pipeline* — a pathway through which people move. The documented trajectory from wellness communities to far-right ideology.

**Origin**
The pipeline was documented by journalists and researchers from around 2018. Research traced how anti-vaccination, anti-pharmaceutical wellness content connected algorithmically to far-right political content through shared enemy frameworks.

> The Wellness-to-Alt-Right Pipeline is the documented algorithmic pathway connecting wellness and spirituality communities to far-right political ideology — through the shared framework of anti-establishment distrust.

**The Appeal**
Health scepticism and distrust of pharmaceutical companies are responses to genuine failures of medical institutions.

**The Friction**
[[public/platform_mechanisms/recommender_systems_public|Recommender Systems]] connect anti-establishment health content to anti-establishment political content: the shared enemy makes the journey structurally smooth. [[public/culture_wars/conspirituality_public|Conspirituality]] is the cultural context.

**Why This Matters**
Once you see the pipeline as an architectural feature of recommendation systems, you see that the route from wellness to alt-right is built into the infrastructure.

**See also:** [[public/culture_wars/conspirituality_public|Conspirituality]] · [[public/platform_mechanisms/recommender_systems_public|Recommender Systems]] · [[public/appropriated_counter_force/digital_detox_public|Digital Detox]]

`#culture-wars-political-language` `#power` `#framing` `#digital-alertness`

---

📖 **Read more**
- [Alt-right pipeline](https://en.wikipedia.org/wiki/Alt-right_pipeline) — Wikipedia

---

*Generated with AI (Claude, Anthropic) using cartographic prompting — a research method developed within Project Digitale Alertheid, HAN CMD, 2026.*
