---
term: Subhuman
cluster: Culture Wars & Political Language
analytical_layer: mechanism
status: publieksversie
version: V1
public_entry: subhuman_public.md
analysis_version: pending
tags:
  - culture-wars-political-language
  - "Culture Wars & Political Language"
  - publieksversie
related_cause:
  - public/platform_mechanisms/attention_economy_public
  - public/platform_mechanisms/recommender_systems_public
related_mechanism:
  - public/culture_wars/dehumanization_public
  - public/platform_mechanisms/ragebaiting_public
related_consequence:
  - []
related_reaction:
  - public/culture_wars/deplatforming_public
---

## Navigation

**Layer:** Mechanism — Dehumanisation; historical origin in genocide discourse

**Cause:** [[public/platform_mechanisms/attention_economy_public|Attention Economy]] · [[public/platform_mechanisms/recommender_systems_public|Recommender Systems]]
**Mechanism:** [[public/culture_wars/dehumanization_public|Dehumanization]] · [[public/platform_mechanisms/ragebaiting_public|Ragebaiting]]
**Consequence:** *—*
**Reaction:** [[public/culture_wars/deplatforming_public|Deplatforming]]

**Public entry:** this document
**Deep analysis:** *not yet created*

---

> 🔍 **Quick Read** — 2 minutes · Public entry · Kabinet van Digitale Termen

---

# Subhuman

The word has a history. Every time it has been used, it has preceded something worse.

**Literal meaning**
*Sub-* — below. *Human* — of the human species. Subhuman explicitly places a person or group below the threshold of humanity.

**Origin**
The term has documented use in genocide discourse — Nazi propaganda, Rwandan radio broadcasts preceding the 1994 genocide. Its contemporary circulation in digital spaces has been documented by hate speech researchers.

> Subhuman is explicit dehumanisation language — a term with documented historical use as a precursor to organised violence, now circulating in digital political discourse.

**The Appeal**
There is no legitimate appeal — the term has no benign use case.

**The Friction**
[[public/culture_wars/dehumanization_public|Dehumanization]] describes the psychological process. Subhuman is the linguistic expression. Research on hate speech documents that dehumanising language precedes and enables violence.

**Why This Matters**
Once you see Subhuman as a historical term with documented consequences, you see that its presence in digital discourse is not a new phenomenon but a continuation of a documented pattern.

**See also:** [[public/culture_wars/dehumanization_public|Dehumanization]] · [[public/culture_wars/great_replacement_public|Great Replacement]]

`#culture-wars-political-language` `#power` `#framing` `#digital-alertness`

---

📖 **Read more**
- [Dehumanization](https://en.wikipedia.org/wiki/Dehumanization) — Wikipedia

---

*Generated with AI (Claude, Anthropic) using cartographic prompting — a research method developed within Project Digitale Alertheid, HAN CMD, 2026.*
