---
term: SMV (Sexual Market Value)
cluster: Gender & Identity
analytical_layer: cause
status: publieksversie
version: V2
analysis_version: pending
tags:
  - gender-identity
  - "Gender & Identity"
  - publieksversie
related_cause:
  - Manosphere
  - Recommender Systems
related_mechanism:
  - Looksmaxxing
  - Hypergamy
related_consequence:
  - Incel
  - Blackpill
  - Comparison Culture
related_reaction:
  - Womanosphere
---

# SMV (Sexual Market Value)


***Attractiveness, status, and income reduced to a single number. The number determines your worth.***

**Literal meaning:** Sexual Market Value is a concept in **manosphere** and red-pill discourse that treats human attractiveness — physical, financial, social, and personality-based — as a quantifiable market position. Men and women are assigned scores, typically on a scale of 1–10, that determine their "market position" in the dating and mating landscape. The concept frames human relationships as market transactions between buyers and sellers of attractiveness.

**Origin:** The concept draws loosely on evolutionary psychology — specifically sexual selection theory — and combines it with market economics and game theory. It circulates heavily in pickup artist, red-pill, and **incel** communities, where it provides the shared analytical framework connecting **looksmaxxing**, **hypergamy**, **incel** ideology, and the alpha/beta hierarchy. Academic researchers including Debbie Ging and Michael Kimmel have documented its role as the conceptual foundation of **manosphere** ideology.

> A framework that reduces human attractiveness and relational worth to a single quantifiable market score — applied as if it were a neutral measurement.

**The Appeal:** SMV provides clarity in a domain that is genuinely confusing: why some people are more romantically successful than others, what determines attraction, and what can be changed. The market metaphor imports the apparent objectivity of economics into an inherently subjective domain. For men trying to understand romantic rejection, the framework offers diagnosis and prescription.

**The Friction:** The market metaphor is not neutral — it transforms persons into positions. [[Looksmaxxing]] is the self-improvement response: raise your score. [[Hypergamy]] — the claim that women always seek partners above their station — is the market dynamic that supposedly explains female behaviour. [[Incel]] is the position of those at the bottom of the market. [[Blackpill]] is the conclusion that the market position is fixed by biology. Once people are market positions, the ethical framework governing relationships changes: deception is strategy, withdrawal is risk management, resentment is rational response to market failure. The concept borrows scientific-sounding language from evolutionary psychology while typically misrepresenting or oversimplifying the underlying research.

**Why This Matters:** SMV is the conceptual foundation of the entire cluster. Once you understand it, the rest of the vocabulary becomes readable as a single system: all the other terms are positions, strategies, or responses within the same market framework.

**See also:** [[Looksmaxxing]] · [[Hypergamy]] · [[Incel]] · [[Blackpill]] · [[Manosphere]]


---

*Generated with AI (Claude, Anthropic) using cartographic prompting — a research method developed within Project Digitale Alertheid, HAN CMD, 2026.*

**Read more:**
- [Alphas, Betas, and Incels](https://doi.org/10.1177/1097184x17706401) — Ging, D. (2019). *Men and Masculinities*
- [Angry White Men](https://www.publicaffairsbooks.com/titles/michael-kimmel/angry-white-men/9781568589619/) — Kimmel, M. (2013). *PublicAffairs*
  [Perceived Motives and Mental Health Characteristics of the **Incel** Men: What **Incel** Men Think Women Value in a Mate vs. Women’s Actual Mate Preferences](https://essay.utwente.nl/81872/1/Kreuzberg_BA_Psychology.pdf) — Kreuzberg, M. S. (2020). _University of Twente_
