---
term: Biometric Surveillance
cluster: Platform Mechanisms & Economics
analytical_layer: mechanism
status: publieksversie
version: V2
analysis_version: pending
related_cause:
  - Surveillance Capitalism
  - Microtargeting
related_mechanism:
  - Algorithmic Violence
  - Deceptive Design
related_consequence:
  - Algorithmic Violence
  - Deadnaming
related_reaction:
  - Brussels Effect
  - Privacy, Data and Control
---

***The camera does not just see you. It measures you — your face, your pace, your pulse — and stores the result.***

**Literal meaning:** **Biometric surveillance** is the collection, processing, and analysis of physical or behavioural characteristics — facial features, fingerprints, iris patterns, gait, voice, heartrate — to identify, track, or make inferences about individuals. In the platform context, it includes facial recognition in consumer devices, emotion detection in advertising systems, and the use of physiological signals as engagement and targeting data.

**Origin:** - [Touch ID](https://support.apple.com/en-us/105095) — Apple Inc. (2013). _Apple Support_
- [Face ID security and privacy overview](https://support.apple.com/en-us/102381) — Apple Inc. (2017). _Apple Support_
- [The Age of Surveillance Capitalism: The Fight for a Human Future at the New Frontier of Power](https://books.google.fr/books/about/The_Age_of_Surveillance_Capitalism.html?id=lRqrDQAAQBAJ&redir_esc=y) — Zuboff, S. (2019). _PublicAffairs_
- [Meta Is Warned That Facial Recognition Glasses Will Arm Sexual Predators](https://www.wired.com/story/meta-ray-ban-oakley-smart-glasses-no-face-recognition-civil-society/) — Klee, M. (2026). _WIRED_
- [Meta's Ray-Ban Smart Glasses Used To Dox Strangers In Public Thanks To AI And Facial Recognition](https://www.forbes.com/sites/johnkoetsier/2024/10/03/metas-ray-ban-smart-glasses-used-to-instantly-dox-strangers-in-public-thanks-to-ai-and-facial-recognition/) — Koetsier, J. (2024). _Forbes_
- [Harvard students used Meta Ray-Bans to do facial recognition. Meta execs once thought this was a good idea.](https://www.businessinsider.com/meta-ray-ban-glasses-facial-recognition-demo-students-2024-10) — Business Insider (2024). _Business Insider_
- [Privacy laws can't keep up with ‘luxury surveillance’](https://www.theverge.com/tech/807834/meta-smart-glasses-privacy-laws-wearables) — Pierce, D. (2025). _The Verge_

> The collection and analysis of physical body signals — face, gait, heartrate — to identify, track, or make inferences about people.

**The Appeal:** Biometric authentication is genuinely convenient and secure. Face ID unlocks a phone faster and more securely than a PIN. Fitness trackers that monitor heartrate provide useful health data. The consumer applications are real and valued.

**The Friction:** The same technology that unlocks your phone can identify you in a public space without your knowledge. [[Surveillance Capitalism]] — human behaviour as raw material — extends to physical presence when biometric data is collected passively. [[Deadnaming]] is one specific consequence: facial recognition systems may identify a trans person by their former name, which is retained in databases they cannot access or correct. [[Algorithmic Violence]] — harm caused by automated systems without individual decisions — describes what happens when biometric systems make consequential errors: misidentification in criminal justice systems has led to wrongful arrests. The EU AI Act (2024) restricts real-time **biometric surveillance** in public spaces — a recognition that the technology requires specific governance. Van Iperen (2026) makes the architecture legible through parallel: the Halter livestock system collects 6,000 data points per animal per minute — location, movement, sleep, temperature, social interaction, eating, excretion — and the smartphone does the same for humans. *"Als een soort halsband leest de telefoon gedrag uit en geeft prikkels terug"*: the phone reads behaviour and returns stimuli. The body is the sensor; the feed, notification, or health advice is the correction signal sent back.

**Why This Matters:** **Biometric surveillance** makes the body readable. Once your face is a data point, your presence in a space generates a record you did not choose to create. The body is no longer private by default.

**See also:** [[Surveillance Capitalism]] · [[Algorithmic Violence]] · [[Deadnaming]] · [[Brussels Effect]] · [[Microtargeting]]


---
**Read more:**
- [Atlas of AI](https://yalebooks.yale.edu/book/9780300209570/atlas-of-ai/) — Crawford, K. (2021). *Yale University Press*
- [Big Tech maakt van burgers vee](https://www.vn.nl/op-afstand-gestuurde-mens) — Van Iperen, R. (2026). *Vrij Nederland*, 9 juli
- - [Touch ID](https://support.apple.com/en-us/105095) — Apple Inc. (2013). _Apple Support_
- [Face ID security and privacy overview](https://support.apple.com/en-us/102381) — Apple Inc. (2017). _Apple Support_
- [The Age of Surveillance Capitalism: The Fight for a Human Future at the New Frontier of Power](https://books.google.fr/books/about/The_Age_of_Surveillance_Capitalism.html?id=lRqrDQAAQBAJ&redir_esc=y) — Zuboff, S. (2019). _PublicAffairs_
- [Meta Is Warned That Facial Recognition Glasses Will Arm Sexual Predators](https://www.wired.com/story/meta-ray-ban-oakley-smart-glasses-no-face-recognition-civil-society/) — Klee, M. (2026). _WIRED_
- [Meta's Ray-Ban Smart Glasses Used To Dox Strangers In Public Thanks To AI And Facial Recognition](https://www.forbes.com/sites/johnkoetsier/2024/10/03/metas-ray-ban-smart-glasses-used-to-instantly-dox-strangers-in-public-thanks-to-ai-and-facial-recognition/) — Koetsier, J. (2024). _Forbes_
- [Harvard students used Meta Ray-Bans to do facial recognition. Meta execs once thought this was a good idea.](https://www.businessinsider.com/meta-ray-ban-glasses-facial-recognition-demo-students-2024-10) — Business Insider (2024). _Business Insider_
- [Privacy laws can't keep up with ‘luxury surveillance’](https://www.theverge.com/tech/807834/meta-smart-glasses-privacy-laws-wearables) — Pierce, D. (2025). _The Verge_

---

## Navigation

**Layer:** Mechanism — the collection and analysis of physical body signals as commercial and surveillance data

**Cause:** [[Surveillance Capitalism]] · [[Microtargeting]]
**Mechanism:** [[Algorithmic Violence]] · [[Deceptive Design]]
**Consequence:** [[Algorithmic Violence]] · [[Deadnaming]]
**Reaction:** [[Brussels Effect]]

<div class="ai-attribution">Created with AI assistance (Claude, ChatGPT, Lumo) using cartographic prompting — a research method developed within Project Digitale Alertheid, HAN CMD, 2026.</div>
