---
term: Attention Harvesting
cluster: Platform Mechanisms & Economics
analytical_layer: mechanism
status: publieksversie
version: V2
analysis_version: pending
tags:
  - platform-mechanisms-economics
  - "Platform Mechanisms & Economics"
  - publieksversie
related_cause:
  - Attention Economy
  - Dopamine Feedback Loops
related_mechanism:
  - Recommender Systems
  - Dark Patterns
related_consequence:
  - Cognitive Surrender
  - Doomscrolling
related_reaction:
  - Calm Technology
  - Digital Detox
---

# Attention Harvesting


***The notification arrives. You pick up the phone. You did not decide to do that.***

**Literal meaning:** **Attention harvesting** describes the systematic extraction of human attention by digital platforms — designing interfaces, notification systems, and content flows to maximise the time and focus users give to a platform, which is then converted into advertising revenue and behavioural data.

**Origin:** The concept develops from Herbert Simon's 1971 observation that information abundance creates attention scarcity, and from Tim Wu's *The Attention Merchants* (2016), which traced the commercial history of attention capture from 19th-century newspapers through broadcast television to digital platforms. The harvesting metaphor — treating attention as a crop to be extracted — is implicit in **Shoshana Zuboff**'s **surveillance capitalism** framework and explicit in Tim Kendall's 2017 congressional testimony about Facebook's engagement-maximisation design.

> The deliberate extraction of human attention through platform design — notifications, variable rewards, and autoplay that convert your focus into advertising revenue.

**The Appeal:** From the platform's perspective, **attention harvesting** funds free services. The user pays with attention rather than money — a transaction that feels frictionless and benefits both parties when the content is genuinely valuable. Advertising-funded media has operated on this logic since the 19th century.

**The Friction:** The asymmetry is the problem. [[Dopamine Feedback Loops]] — variable reward schedules — are the psychological mechanism: attention is harvested by making disengagement harder than continued engagement. [[Recommender Systems]] are the technical infrastructure: they optimise for dwell time and return visits, not for user wellbeing. [[Dark Patterns]] — interface design that misleads — are the specific design tools. James Williams, in *Stand Out of Our Light* (2018), argues that **attention harvesting** is not merely commercially inconvenient — it structurally undermines the capacity for autonomous agency, because agency requires sustained attention that the platform systematically depletes.

**Why This Matters:** **Attention harvesting** reframes the relationship between user and platform. The user is not the customer — they are the product. Once you know the transaction, "free" platform means something different.

**See also:** [[Attention Economy]] · [[Dopamine Feedback Loops]] · [[Recommender Systems]] · [[Dark Patterns]] · [[Cognitive Surrender]]


---

*Generated with AI (Claude, Anthropic) using cartographic prompting — a research method developed within Project Digitale Alertheid, HAN CMD, 2026.*

**Read more:**
- [Designing Organizations for an Information-Rich World](https://zeus.zeit.de/2007/39/simon.pdf) — Simon, H.A. (1971). In M. Greenberger (Ed.), _Computers, Communication, and the Public Interest_. Johns Hopkins University Press.
- [The Attention Merchants: The Epic Scramble to Get Inside Our Heads](https://scholarship.law.columbia.edu/books/64/) — Wu, T. (2016). _Knopf_
- [Stand Out of Our Light: Freedom and Resistance in the Attention Economy](https://www.cambridge.org/core/books/stand-out-of-our-light/3F8D7BA2C0FE3A7126A4D9B73A89415D) — Williams, J. (2018). _Cambridge University Press_
- [The Age of Surveillance Capitalism: The Fight for a Human Future at the New Frontier of Power](https://books.google.fr/books/about/The_Age_of_Surveillance_Capitalism.html?id=lRqrDQAAQBAJ&redir_esc=y) — Zuboff, S. (2019). _PublicAffairs_
  [Shoshana **Zuboff**, The age of **surveillance capitalism**: the fight for a human future at the new frontier of power](https://www.researchgate.net/publication/346844216_Shoshana_Zuboff_The_age_of_surveillance_capitalism_the_fight_for_a_human_future_at_the_new_frontier_of_power_New_York_Public_Affairs_2019_704_pp_ISBN_978-1-61039-569-4_hardcover_978-1-61039-270-0_eboo) — Hongladarom, S. (2020). _AI & Society_

---

## Navigation

**Layer:** Mechanism — the systematic extraction of human attention as a commercial resource

**Cause:** [[Attention Economy]] · [[Dopamine Feedback Loops]]
**Mechanism:** [[Recommender Systems]] · [[Dark Patterns]]
**Consequence:** [[Cognitive Surrender]] · [[Doomscrolling]]
**Reaction:** [[Calm Technology]] · [[Digital Detox]]
